/*
  ...
  Trigger has a variable (_triggerable) that points to a game object which has a component which inherits from Trigger.
  
  When the player collides with a GameObject that contains one of these Triggers as a component, it will fire TriggerEvent()
  in the associated Triggerable inheriting component of the thing the player hit. With the player GameObject as the first argument.
  
  Player hits this trigger > trigger fires custom code inside another object
