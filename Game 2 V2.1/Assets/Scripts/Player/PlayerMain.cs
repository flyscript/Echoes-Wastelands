...
public class PlayerMain : MonoBehaviour
{
		...
		if (!_dead)
		{
			...
			if (Input.GetKeyDown(KeyCode.Space))
			{
				if (_ccs.Grounded)
				{
					...
					_ccs._canDoubleJump = true;
					_ccs._jumpedSinceGrounded = true; 
				}
				//Double jump.
				else if (_ccs._canDoubleJump || !_ccs._jumpedSinceGrounded)
				{
					_ccs._canDoubleJump = false;
					_ccs._jumpedSinceGrounded = true;
					/*
					...
					GetComponent<Rigidbody2D>().AddForce(new Vector2(0, (GetComponent<Rigidbody2D>().velocity.y*GetComponent<Rigidbody2D>().mass*_massMagnitude)+_ccs.JumpForce));
				}
			}
	...
	private void OnTriggerEnter2D(Collider2D other)
	{
		...
		else if (other.GetComponent<Spike>() != null || other.GetComponent<RollingRock>() != null || other.GetComponent<AppearingSpike>() != null)
		{
			Damage(500);
		}
	...
	private void Die()
	{
		...
		SceneManager.LoadScene("Game 2 1.8");
	}
