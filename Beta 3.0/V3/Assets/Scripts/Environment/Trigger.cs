...
public class Trigger : MonoBehaviour
{
	...
	[SerializeField]
	private float _waitUntilTriggering;
	//Setup active bool
	[SerializeField]
	...
	public void Fire(GameObject player)
	{
		...
		StartCoroutine(FireTrigger(player));
	}
	
	IEnumerator FireTrigger(GameObject player)
	{
		yield return new WaitForSeconds(_waitUntilTriggering);
		...
		if (_triggerable != null && _triggerable.GetComponent<Triggerable>() && _active)
		{
			...
			_triggerable.GetComponent<Triggerable>().TimeActivated = Time.fixedTime;
	...
	public void TriggerSpikeAnimation()
	{
