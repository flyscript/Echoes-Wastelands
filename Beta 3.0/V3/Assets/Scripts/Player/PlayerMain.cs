...
public class PlayerMain : MonoBehaviour
{
	...
	void Update ()
	{
		...
		if (!_dead)
		{
			...
			//Immortality
			if (_immortal && Time.time - _timeStartedImmortal > _immortalityTime)
			{
				_immortal = false;
				_health = 100f;
			}
		...
		//Santokh - Show death particle if player dies
		if (_dead && deathParticle.activeInHierarchy == false)
		{
	...
	private void Die()
	{
		...
		Camera.main.GetComponent<CameraBehaviour>().Shake(_deathShakeIntensity, _deathShakeLength);
		//Camera.main.GetComponent<ScreenShake>().Shake(_deathShakeIntensity, _deathShakeLength);
		
		StartCoroutine(LoadLevelAfterSeconds(SceneManager.GetActiveScene().name, 2f));
//		StartCoroutine(LoadLevelAfterSeconds(0.5f));
	}
	...
	/// <summary>
	/// Method to retrieve an item from inventory using a name
	/// </summary>
	/// <param name="itemName">Name of item to retrieve</param>
